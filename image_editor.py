...
def app():
    ...
    if img_file:
        ...
        st.title("FILTERS")
        option = st.selectbox("...", ("none", 'SHARPEN', 'SMOOTH',
                                      'EMBOSS', 'CONTOUR', 'BLUR', 'BOX BLUR', 'EDGE ENHANCE'))
        if option == 'SHARPEN':
        ...
        choice = st.selectbox("choose font", ("OdibeeSans-Regular",
                                              "RobotoMono-VariableFont_wght", "ZCOOLKuaiLe-Regular"))
        f = st.number_input("enter font size")
        if choice == "RobotoMono-VariableFont_wght":
            font = ImageFont.truetype(
                'RobotoMono-VariableFont_wght.ttf', int(f))
        ...
        option2 = st.selectbox(
            "", ("none", 'FLIP_LEFT_RIGHT', 'FLIP_TOP_BOTTOM', 'ROTATE_90'))
